...
import com.jzd.jzshop.databinding.FragmentInCommisWithdrawBinding;
 ...
 */
public class InCommisWithdrawFragment extends BaseFragment<FragmentInCommisWithdrawBinding, InCommisWithdrawViewModel>
        implements OnRefreshListener, OnLoadmoreListener {
    private int page = 1;   //分页页码
    private boolean isRefresh = true; //是否是刷新
    private InCommisWithdrawListAdapter adapter;

    public InCommisWithdrawFragment() {

    }

    @Override
    public int initContentView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        return R.layout.fragment_in_commis_withdraw;
    }

    @Override
    public int initVariableId() {
        return BR.inCommisWithdrawVM;
    }

    @Override
    public InCommisWithdrawViewModel initViewModel() {
        AppViewModelFactory factory = AppViewModelFactory.getInstance(getActivity().getApplication());
        return ViewModelProviders.of(this, factory).get(InCommisWithdrawViewModel.class);
    }
    ...
    public void initData() {
        ...
        viewModel.requestData(binding.refreshLayout, "1",page, isRefresh);
    ...
    public void initViewObservable() {
        ...
        viewModel.uc.mLiveData.observe(this, new Observer<List<CommisWithdraRecordEntity.DataBean>>() {
            ...
            public void onChanged(@Nullable List<CommisWithdraRecordEntity.DataBean> result) {
                ...
                if (adapter==null){
                    adapter = new InCommisWithdrawListAdapter(getActivity(),
                            result, R.layout.item_rv_in_commis_withdraw_list);
    ...
    private void initMallRefresh() {
        ...
        binding.mallRefreshHeader.setDrawableSize(16);
        binding.mallRefreshHeader.setTitleTextColors(R.color.white);
        binding.mallRefreshHeader.setTitleTimeColors(R.color.white);//同时设置箭头和图片的大小（dp单位）
        ...
        binding.mallRefreshFooter.setDrawableSize(16);
        binding.mallRefreshHeader.setmArrowViewWhite();
        ProgressDrawable progressDrawable = binding.mallRefreshHeader.getmProgressView();
        binding.mallRefreshHeader.setProgressDrawable(progressDrawable);//同时设置箭头和图片的大小（dp单位）
