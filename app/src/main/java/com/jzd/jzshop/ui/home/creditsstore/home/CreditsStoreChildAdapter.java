 ...
 * @date :2020/5/8 17:45
 */
public class CreditsStoreChildAdapter extends SuperAdapter<CreditsStoreEntity.ResultBean.DataBeanX.DataBean> {
    private static final String TAG = CreditsStoreChildAdapter.class.getSimpleName();
    private Context mContext;
    private List<CreditsStoreEntity.ResultBean.DataBeanX.DataBean> dataList;
    private String activityType = "";

    public CreditsStoreChildAdapter(Context context, String title, List<CreditsStoreEntity.ResultBean.DataBeanX.DataBean> items, int layoutResId) {
    ...
    public void onBind(SuperViewHolder holder, int viewType, int layoutPosition, CreditsStoreEntity.ResultBean.DataBeanX.DataBean item) {
        ...
        if (!TextUtils.isEmpty(activityType)){
            if (activityType.equals("抽奖专区")){//抽奖
